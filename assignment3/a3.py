...
def is_valid_word(wordlist, word):
    ...
    True
    >>> is_valid_word(['ROB', 'AND', 'OR', 'IF'], 'AND')
    True
    >>> is_valid_word(['ROB', 'AND', 'OR', 'IF'], 'KOU')
    False
    """
    if word in wordlist:
        return True
...
def make_str_from_column(board, column_index):
    ...
    'NS'
    >>> make_str_from_column([['A', 'N', 'T', 'T'], ['X', 'S', 'O', 'B'], ['D', 'X', 'S', 'P']], 1)
    'NSX'
    """
    word = ''
    for mylist in board:
        word = word + mylist[column_index]
    return word
    # i = 0
    # word = ''
...
board_file = open('assignment3/board1.txt', 'r')
def read_board(board_file):
    ...
    """
    # .strip() is removing whitespaces in every line
    # and .split() splits every line in list
    # board = [line.strip().split() for line in board_file]
    # return board
